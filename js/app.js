(() => {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".sale-product-slider")) new core(".sale-product-slider__slider", {
            modules: [ Navigation, Pagination ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
            ...
            slideWidth: 300,
            pagination: {
                el: ".sale-product-slider__dotts",
                clickable: true,
                dynamicBullets: true,
                dynamicMainBullets: 3
            },
            navigation: {
                prevEl: ".sale-product-slider__prev",
                nextEl: ".sale-product-slider__next"
            },
            breakpoints: {
                560: {
                    slidesPerView: 1,
        ...
        if (document.querySelector(".product-item-card")) {
            let thumbsSwiper;
            thumbsSwiper = new core(".thumbs-images__slider", {
    ...
    da.init();
    const img = document.querySelector(".media-block__image img");
    const leafs = document.querySelector(".media-block__leafs");
    if (img) {
        function windowChangeWatcher() {
            function handleResize() {
                const screenWidth = window.innerWidth;
                if (screenWidth > 991) {
                    const style = window.getComputedStyle(img);
                    const maxWidth = style.getPropertyValue("max-width");
                    setLeafsWidth(maxWidth);
                } else window.removeEventListener("resize", handleResize);
            }
            window.addEventListener("resize", handleResize);
        }
        function setLeafsWidth(maxWidth) {
            leafs.style.width = maxWidth;
        }
        windowChangeWatcher();
    }
    const buttonUp = document.querySelector(".header-footer__back-to");
    buttonUp.addEventListener("click", (() => {
        const currentPosition = window.pageYOffset;
        const duration = 2e3;
        const scrollStep = -currentPosition / (duration / 60);
        const scrollAnimation = setInterval((() => {
            const newPosition = window.pageYOffset + scrollStep;
            window.scrollTo(0, newPosition);
            if (newPosition <= 0) clearInterval(scrollAnimation);
        }), 16.7);
    }));
    window["FLS"] = true;
    isWebp();
